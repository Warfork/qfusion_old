...
static void Shader_MakeCache( qboolean silent, char *name, char *path );
...
static void Shaderpass_CubeMapExt( shader_t *shader, shaderpass_t *pass, int addFlags, char **ptr )
{
	int flags;
	char *token;
...
static void Shaderpass_CubeMap( shader_t *shader, shaderpass_t *pass, char **ptr ) {
	Shaderpass_CubeMapExt( shader, pass, IT_CLAMP, ptr );
}
...
void R_InitShaders( qboolean silent )
{
	...
	char *shaderbuf2;
	size_t shaderbuflen, shaderbuf2len;
	...
	for( i = 0; i < numfiles; i++, fileptr += filelen + 1, dirptr += dirlen + 1 ) {
		...
		if( fileptr )
			Shader_MakeCache( silent, fileptr, dirptr );
	}
...
static void Shader_MakeCache ( qboolean silent, char *name, char *path )
{
	...
	Q_snprintfz( basepath, sizeof(basepath), "%s/%s/", fs_basepath->string, fs_basedir->string );
	...
	do {
		...
		if ( cache ) {
			...
			// or both shaders come from game directory
			if( cache->basedir || cache->basedir == basedir ) {
				cache->path = name;
