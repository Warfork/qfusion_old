...
{
	if (gl_nobind->value)		// performance evaluation option
...
void LoadJPG (char *name, byte **pic, int *width, int *height)
{
	...
	*pic = NULL;
    ...
    if (cinfo.output_components != 3) {
		Com_Printf ("Bad number of jpg components");
...
void GL_Upload32 (unsigned *data, int width, int height, int flags)
{
	...
	if (!(flags & IT_NOPICMIP) && !(flags & IT_NOMIPMAP))
	{
	...
	if (scaled_width > gl_maxtexsize)
		scaled_width = gl_maxtexsize;
	if (scaled_height > gl_maxtexsize)
		scaled_height = gl_maxtexsize;
	...
	// IT_FOG never reaches this point
	...
	GL_LightScaleTexture (scaled, scaled_width, scaled_height, (flags & IT_NOMIPMAP) );
...
int Draw_GetPalette (void)
{
	...
	for (i=0 ; i<256 ; i++)
	{
		...
		d_8to24floattable[i][0] = r * (1.0/255.0);
		d_8to24floattable[i][1] = g * (1.0/255.0);
		d_8to24floattable[i][2] = b * (1.0/255.0);
	}
