...
================
*/
#define DUMP_PROGRAM(p) { int file; if( FS_FOpenFile( va("programs/%s.glsl", #p), &file, FS_WRITE ) != -1 ) { FS_Write( r_##p, strlen( r_##p ), file ); FS_FCloseFile( file ); } }
...
void R_UpdateProgramUniforms( int elem, vec3_t eyeOrigin,
							 ...
							 superLightStyle_t *superLightStyle, qboolean frontPlane, int TexWidth, int TexHeight,
