...
cgs_skeleton_t *UI_SkeletonForModel( struct model_s *model )
{
	...
	buffer = UI_Malloc( sizeof( cgs_skeleton_t ) + numBones * sizeof( cgs_bone_t ) +
...
qboolean UI_SkeletalUntransformedPoseLerpAttachment( orientation_t *orient, cgs_skeleton_t *skel, bonepose_t *boneposes, bonepose_t *oldboneposes, float backlerp, char *bonename )
{
	...
	tr_boneposes = UI_Malloc ( sizeof(bonepose_t) * skel->numBones );
	UI_TransformBoneposes( skel, tr_boneposes, boneposes );
	tr_oldboneposes = UI_Malloc ( sizeof(bonepose_t) * skel->numBones );
...
void UI_InitTemporaryBoneposesCache( void )
{
	...
	TBC = UI_Malloc ( sizeof(bonepose_t) * TBC_Size );
	TBC_Count = 0;
}
...
//===============
void UI_ExpandTemporaryBoneposesCache( void )
{
	bonepose_t *temp;

	temp = TBC;

	TBC = UI_Malloc ( sizeof(bonepose_t) * (TBC_Size + TBC_Block_Size) );
...
//===============
bonepose_t *UI_RegisterTemporaryExternalBoneposes( cgs_skeleton_t *skel, bonepose_t *poses )
{
