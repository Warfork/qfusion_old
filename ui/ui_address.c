...
void AddressBook_MenuInit( void )
{
	...
	for ( i = 0; i < NUM_ADDRESSBOOK_ENTRIES; i++ )
	{
		...
		s_addressbook_fields[i].generic.y		= y + i*y_offset;
		s_addressbook_fields[i].generic.localdata[0] = i;
		s_addressbook_fields[i].cursor			= 0;
		s_addressbook_fields[i].length			= 60;
		s_addressbook_fields[i].visible_length	= 30;

		strcpy( s_addressbook_fields[i].buffer, trap_Cvar_VariableString( buffer ) );
...
const char *AddressBook_MenuKey( int key )
{
	if ( key == K_ESCAPE )
	{
		...
		for ( index = 0; index < NUM_ADDRESSBOOK_ENTRIES; index++ )
		{
			...
			trap_Cvar_Set( buffer, s_addressbook_fields[index].buffer );
		}
	}
