...
struct sfx_s *CG_RegisterSexedSound ( int entnum, char *name )
{
	...
	Com_sprintf ( sexedFilename, sizeof(sexedFilename), "players/%s/%s", model, name+1 );

	// so see if it exists
	if ( trap_FS_FileExists (sexedFilename) ) {	// yes, register it
		sexedSfx->sfx = trap_S_RegisterSound ( sexedFilename );
	} else {		// no, revert to the male sound
		Com_sprintf ( sexedFilename, sizeof(sexedFilename), "sound/player/%s/%s", "male", name+1 );
		sexedSfx->sfx = trap_S_RegisterSound ( sexedFilename );
	}
...
void CG_LoadClientInfo ( cg_clientInfo_t *ci, char *s, int client )
{
	...
	if ( cg_noSkins->value || *s == 0 ) {
		...
		Com_sprintf ( skin_filename, sizeof(skin_filename), "players/male/grunt_default.skin" );
		...
		*t = 0;
		...
		if ( !ci->model ) {	// model file
			...
			Com_sprintf ( skin_filename, sizeof(skin_filename), "players/%s/%s_default.skin", model_name, skin_name );
			ci->skin = trap_R_RegisterSkinFile ( skin_filename );
			if ( !ci->skin ) {
			...
			if ( !ci->skin && !ci->shader ) {
				// see if the skin exists for the male model
				Com_sprintf ( skin_filename, sizeof(skin_filename), "players/%s/grunt_default.skin", model_name, skin_name );
				ci->skin = trap_R_RegisterSkinFile ( skin_filename );
				if ( !ci->skin ) {
		...
		for ( i = 0; i < cgs.numWeaponModels; i++ ) {
			Com_sprintf ( weapon_filename, sizeof(weapon_filename), "players/%s/%s", model_name, cgs.weaponModels[i] );
			ci->weaponmodel[i] = trap_R_RegisterModel ( weapon_filename );

			if ( !ci->weaponmodel[i] && strcmp(model_name, "cyborg") == 0 ) {
				// try male
				Com_sprintf ( weapon_filename, sizeof(weapon_filename), "players/male/%s", cgs.weaponModels[i] );
				ci->weaponmodel[i] = trap_R_RegisterModel ( weapon_filename );
			}
			if ( !cg_vwep->value ) {
